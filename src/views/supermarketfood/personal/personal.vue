<template>
    <div id="personal" style="text-align: center;">
        <div class="annimate">
           <h3>
  <svg version="1.1" class="subtitle" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 229.2 69.6" style="enable-background:new 0 0 229.2 69.6;" xml:space="preserve">
        ...
        <div style="margin-top: 1em">
            <el-upload action="#" list-type="picture-card" :auto-upload="false" :limit="1" :on-change="hello"
                :file-list="fileList">
                <i slot="default" class="el-icon-plus"></i>
                <div slot="file" slot-scope="{file}">
                    <img class="el-upload-list__item-thumbnail" :src="fileList.url" alt="">
        ...
        <div style="margin-top:1.2em;margin-bottom: 100px;">
            <p>
                用户名为：{{userInfo.account}}
            </p>
        </div>
        <div style="margin-left: 300px;margin-right: 300px;margin-top:-3em">
            <span>设置新密码：</span><br/><br/>
            <el-input v-model.lazy="form.password" placeholder="请输6~15位密码"></el-input><br/><br/>
            <span v-if="form.password" style="color: red;">{{passwordValidate.errorText}}</span>
            <span style="margin-top:1em">确认新密码：</span><br/><br/>
            <el-input v-model="form.checkedPassword" placeholder="请再输入一遍密码"></el-input><br/><br/>
            <span v-if="form.checkedPassword" style="color: red;">{{passwordCheckValidate.errorText}}</span>
        </div>
        <el-button size="small" type="primary" @click="SaveOrUpdate">保存信息</el-button>
    </div>
</template>
<script>
    ...
    export default {
        data() {
            return {
                file: "",
                fileList: [{ name: 'bird.jpeg', url: 'https://fuss10.elemecdn.com/e/5d/4a731a90594a4af544c0c25941171jpeg.jpeg' }],
                //personalDate:[],
                form: {
                    checkedPassword:""
                },
        ...
        mounted() {
            ...
            if (this.userInfo) {
                ...
                this.fileList = [{ name: this.userInfo.headPortrait, url: 'http://192.168.1.73:8889/image/' + this.userInfo.headPortrait }];
            }
        },
        computed: {
            ...
            passwordValidate: function () {
                var errorText;
                if (!/^[0-9A-Za-z]{6,15}$/.test(this.form.password)) {
                    errorText = '密码少于6位'
                } else if (this.form.password == this.userInfo.password) {
                    errorText = '密码不能与原密码相同'
                }
                else {
                    errorText = ''
                }
            ...
            passwordCheckValidate: function () {
                var errorText;
                if (!/^[0-9A-Za-z]{6,15}$/.test(this.form.checkedPassword)) {
                    errorText = '密码少于6位'
                } else if (this.form.checkedPassword !== this.form.password) {
                    errorText = '两次密码不匹配'
                }
                else if((this.form.checkedPassword == this.userInfo.password)){
                    errorText = '密码不能与原密码相同'
                }
        ...
        methods: {
            ...mapActions("personal", ["updatePersonalInfo"]),
            ...
            SaveOrUpdate() {
                ...
                this.updatePersonalInfo({ userInfo:this.userInfo,password: this.form.checkedPassword, file: this.file });
